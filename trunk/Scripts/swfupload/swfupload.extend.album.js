...
//文件成功队列后运行
HANDLERS.file_queued_handler = function(file){total++;};

//文件队列结束后执行
HANDLERS.file_dialog_complete_handler = function(){
	if(this.Status().queued>0)Util("Upload_Processer","已成功添加" + this.Status().queued + "张照片。");
	this.startUpload();	
};
...
HANDLERS.upload_progress_handler = function(file, bytesComplete, bytesTotal,uploadTotalBytes,totalBytes,speed,timeused){
	...
	Util("Upload_Processer","正在上传[" + file.name + "]，已上传" + percent + "%，速度：" + Util.formatBytes(speed) + "/秒，已用时：" + Util.timeString(parseFloat(timeused/1000),true));
